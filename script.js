...
// console.log('------------ForOF------------', movementsUSDfor);
...
const avg1 = calcAverageHumanAge([5, 2, 4, 1, 15, 8, 3]);
const avg2 = calcAverageHumanAge([16, 6, 10, 5, 6, 1, 4]);
console.log("Chaining Challenge",avg1, avg2);
...
// Strings
const owners = ['Jonas', 'Zach', 'Adam', 'Maratha'];
...
labelBalance.addEventListener('click', function () {
  ...
  const Map =movementsUI2.map(el => Number(el.textContent.replace('💶', '')));
  console.log("MAp",Map);
});
*/
...
// 3
// To Create a Brand New Object Based On The Reduced Function
...
const convertTitleCase = function (title) {
  ...
  const ecxeptions = ['a', 'an', 'and', 'the', 'but', 'or', 'on', 'in', 'with'];
  const titleCase = title
    .toLowerCase()
    .split(' ')
    .map(word => (ecxeptions.includes(word) ? word : capitialize(word)))
    .join(' ');
